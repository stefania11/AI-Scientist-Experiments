...
def train(dataset="shakespeare_char", out_dir="run_0", seed_offset=0):
    ...
    batch_size = 16  # reduced for faster CPU training
    ...
    eval_iters = 100  # reduced eval iterations
    eval_only = False  # if True, script exits right after the first eval
    always_save_checkpoint = True  # save checkpoints for plotting
    never_save_checkpoint = False  # enable checkpoint saving
    # model
    n_layer = 4  # reduced from 6 for CPU training
    n_head = 4   # reduced from 6 for CPU training
    n_embd = 256  # reduced from 384 for CPU training
    dropout = 0.0  # for determinism
    bias = False  # do we use bias inside LayerNorm and Linear layers?
    # adamw optimizer
    learning_rate = 1e-3 if dataset == "shakespeare_char" else 5e-4
    max_iters = 50  # ultra-light for initial testing
    weight_decay = 1e-1
    beta1 = 0.9
    ...
    lr_decay_iters = 45  # should be max_iters - warmup_iters
    ...
    dtype = (
        ...
        else "float32"  # Use float32 for CPU
    )  # 'float32', 'bfloat16', or 'float16', the latter will auto implement a GradScaler
    compile = True if torch.cuda.is_available() else False  # only compile on CUDA
